...
public class EnemyStatusEffects : MonoBehaviour
{
    ...
    private ResourceSnapshot CaptureResourceSnapshot()
    {
        ...
        return new ResourceSnapshot
        {
            ...
            energy = TryGetFirstFloat(target, "energy", "currentEnergy", "sprint", "currentSprint", "stamina"),
            maxEnergy = TryGetFirstFloat(target, "maxEnergy", "maxSprint", "maxStamina")
        };
    }
    ...
    private void SafeHeal(float amount)
    {
        ...
        {
            Debug.LogError($"[EnemyStatusEffects] SafeHeal exception: {ex.Message}");
        }
    }
    ...
    private void SafeRestoreMana(float amount)
    {
        ...
        {
            Debug.LogError($"[EnemyStatusEffects] SafeRestoreMana exception: {ex.Message}");
        }
    }
    ...
    private void SafeRestoreEnergy(float amount)
    {
        ...
        try
        {
            ...
            float current = TryGetFirstFloat(target, "energy", "currentEnergy", "sprint", "currentSprint", "stamina");
            float max = TryGetFirstFloat(target, "maxEnergy", "maxSprint", "maxStamina");
            float newValue = Mathf.Clamp(current + amount, 0f, max);
            TrySetFirstFloat(target, newValue, "energy", "currentEnergy", "sprint", "currentSprint", "stamina");
        }
        catch (Exception ex)
        {
            Debug.LogError($"[EnemyStatusEffects] SafeRestoreEnergy exception: {ex.Message}");
        }
    }
    ...
    private bool TrySetCurrentResourceValue(string resourceKey, float value)
    {
        ...
        switch (resourceKey)
        {
            ...
            case "energy":
                return TrySetFirstFloat(target, value, "energy", "currentEnergy", "sprint", "currentSprint", "stamina");
            default:
                return false;
        }
    }
