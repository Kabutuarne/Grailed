...
public class PlayerStatusEffects : MonoBehaviour
{
    ...
    public class Effect
    {
        ...
        // duration > 0 = timed, duration == 0 = instant, duration < 0 = toggle/infinite
        ...
        // temporary attributes while active
    ...
    public void AddEffect(Effect effect)
    {
        ...
        // Instant effect: apply once and do not store.
        if (effect.duration == 0f)
        {
    ...
    private void SyncResourcesAfterAttributeChange(ResourceSnapshot before)
    {
        ...
        // Let the stats object recalc first, then inspect the new max values.
        SafeClampResourcesToMax();
    ...
    {
        if (Mathf.Approximately(beforeMax, afterMax))
            return;

        float desiredCurrent;

        // Max increased: keep the resource "filled" by the gained max amount.
        // Example: 100/100 -> 200/200 when intelligence raises mana max by 100.
        if (afterMax > beforeMax)
        {
            desiredCurrent = beforeCurrent + (afterMax - beforeMax);
        }
        // Max decreased: never allow above new max.
        // Example: 50/100 -> 10/10 if the new max becomes 10.
        else
        {
            desiredCurrent = Mathf.Min(beforeCurrent, afterMax);
        }
    ...
    private object GetStatsTarget()
    {
        ...
        if (enemyStats != null) return enemyStats;
    ...
    private void SafeRestoreEnergy(float amount)
    {
        ...
        {
            Debug.LogError($"[StatusEffects] SafeRestoreEnergy exception: {ex.Message}");
        }
    }
    ...
    private void SafeClampResourcesToMax()
    {
        try
        {
            if (stats != null)
                ...
                enemyStats.ClampResourcesToMax();
        }
        catch
        {
        }
    ...
    private float TryGetFirstFloat(object target, params string[] memberNames)
    {
        ...
        {
            if (TryGetFloatMember(target, name, out float value))
                return value;
        }
    ...
    private bool TrySetCurrentResourceValue(string resourceKey, float value)
    {
        ...
        {
            case "health":
                return TrySetFirstFloat(target, value, "health", "currentHealth", "currentHP", "hp");
            case "mana":
                return TrySetFirstFloat(target, value, "mana", "currentMana");
            case "energy":
                return TrySetFirstFloat(target, value, "energy", "currentEnergy", "sprint", "currentSprint");
            default:
                return false;
        }
    }
    ...
    private bool TrySetFirstFloat(object target, float value, params string[] memberNames)
    {
        foreach (string name in memberNames)
        {
            if (TrySetFloatMember(target, name, value))
                return true;
        }
